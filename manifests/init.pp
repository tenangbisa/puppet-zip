# @summary Manages zip module's dependencies.
#
...
class zip (
  Optional[String[1]]                       $seven_zip_name     = $zip::params::seven_zip_name,
  Optional[Enum['chocolatey','windows','']] $seven_zip_provider = $zip::params::seven_zip_provider,
  ...
  if $aws_cli_install {
    ...
    if $facts['os']['family'] != 'Windows' {
      ...
      exec { 'install_aws_cli':
        ...
        refreshonly => true,
        subscribe   => zip['awscli-bundle.zip'],
      }
    }
  }
  ...
  if $gsutil_install {
    ...
    if $facts['os']['family'] != 'Windows' {
      ...
      exec { 'install_gsutil':
        ...
        refreshonly => true,
        subscribe   => zip['gsutil.zip'],
      }
    }
  }

  $zips.each |$zip_name, $zip_settings| {
    zip { $zip_name:
      * => $zip_settings,
    }
  }
